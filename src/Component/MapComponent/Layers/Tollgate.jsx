...
export default class Tollgate extends Component {
    ...
    loadFeature = (item) => {
        ...
        if ('9500' === mapType) {
            ...
            //8500Map服务时，卡口点位部分来自/VIID/query/tollgate（真实坐标）；部分来自/api/marker/client（火星坐标）
            //根据undefined === item.Id可判断是/VIID/query/tollgate接口数据，需转化为火星坐标
            position = mapDom.getWGS2Mars(...position);
        }
        Object.assign(item, {
            Lng: position[0],
    ...
    getFeatureStyle = (item) => {
        return new ol.style.Style({
            ...
            text: new ol.style.Text({
                offsetX: 0,
                offsetY: -10,
                text: item.MarkerName || item.MarkerCode,
    ...
    */
    getClusterStyle = (size) => {
        ...
        let style = new ol.style.Style({
            image: new ol.style.Icon({
                src: require(`../Image/tollgateCluster${iconName}.png`),
                anchorXUnits: 'fraction',
                anchorYUnits: 'pixels',
                crossOrigin: 'anonymous',
                scale: 1, //标注图标大小
    ...
    loadGisCluster = (param, layerCode) => {
        getGisCluster(param, 'Tollgate', (data) => {
    ...
    /**
     * 获取卡口点位的小图标（精灵图及其图片位置）
     * @param {Object} item 点位数据
     */
    getPointImgOffset = (item) => {
        const { iconName } = this.props;
        if (!iconName[item.LayerType]) {
            return { image: tollgateImg, offset: [] };
        }
        //获取精灵图中对应小图标的位置
        let startY = (iconName[item.LayerType] % 30000) * 24;
        //卡口状态全默认显示在线，不做以下判断：
        //let startX = 1 === item.Status ? 0 : (2 === item.Status ? 24 : 48)
        let startX = 0;
        return { image: tollgateImg, offset: [startX, startY] };
    }
    //#endregion
    ...
    render() {
        ...
        return (
            <TollgateMenu mapDom={mapDom} layers={layers} />
        );
    }
}